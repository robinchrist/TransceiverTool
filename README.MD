...
A self-contained browser editor is available in [`web/`](web/README.md), with graphical schema-driven
configuration, local WebAssembly conversion, and JSON/binary downloads. Build instructions and tests
are documented in the web README.
...
The JSON schemas in `schemas/` have no header because JSON has no comment syntax; they are covered by
this file and by [LICENSE](LICENSE).

Releases up to and including the last MIT-licensed commit remain available under the MIT license.
...
`.github/workflows/ci.yml` builds on Linux, macOS and Windows for every push and pull request and runs
`.github/smoke-test.sh`, which does byte-exact roundtrips over random data for all three binary types
through both the file and the stdin input path. `.github/workflows/release.yml` publishes binaries when a
`v*` tag is pushed; the same smoke test gates every artifact, so a broken binary is never published.

### Cutting a release

The version is compiled into the binary from `project(... VERSION ...)` in `CMakeLists.txt`, which is the
single source of truth. The tag only names the archives, so **bump `CMakeLists.txt` first, then tag the
commit that contains the bump**:

```
# edit CMakeLists.txt: project ("Transceiver Tool" VERSION 0.2.0)
git commit -am "Release 0.2.0"
git tag v0.2.0 && git push origin master v0.2.0
```

A `check-version` job compares the tag against `CMakeLists.txt` before anything builds and fails the run
if they disagree, so a mismatch cannot be published. Running the workflow manually
(Actions -> Release -> Run workflow) builds and smoke tests every artifact without publishing.

| Artifact | Built on | Notes |
| --- | --- | --- |
| `linux-x86_64`, `linux-aarch64` | Debian 11 container | old glibc/libstdc++ baseline, nothing statically linked |
| `macos-universal` | macos-13 + macos-14, merged with `lipo` | one binary for Intel and Apple Silicon |
| `windows-x86_64` | windows-2022, MSVC | dynamic CRT, see the Windows note below |

### Windows

Nothing is statically linked, on any platform. On Linux and macOS that costs nothing, because the C
and C++ runtimes ship with the operating system. Windows is different: the MSVC runtime
(`VCRUNTIME140.dll`, `MSVCP140.dll`) is not part of Windows, so the binary needs the
[Visual C++ Redistributable](https://aka.ms/vs/17/release/vc_redist.x64.exe) installed. A clean
Windows install does not have it and the program will refuse to start with a missing DLL error until
it does. Most machines already have it, because plenty of other software installs it.

Building on an older toolchain does not avoid this the way it does on Linux - it only changes which
redistributable version is required.

### Linux compatibility

The Linux binaries are built inside Debian 11 rather than on the runner image, so they keep working on
distributions older than the runner. The resulting binary needs `GLIBC_2.14` and `GLIBCXX_3.4.26`,
so libstdc++ (GCC 9) is the limit, not glibc. Verified:

| Runs | Does not run |
| --- | --- |
| Debian 11, Debian 12, Ubuntu 20.04, Ubuntu 22.04, Ubuntu 24.04, Rocky 9, Fedora 41 | Debian 10, CentOS 7 |

If you ever need to go below that, adding `-static-libstdc++ -static-libgcc` drops the `GLIBCXX`
requirement entirely and leaves only `GLIBC_2.14`.

Bullseye is end of life, so the release job installs from `archive.debian.org` and pins it above
priority 1000, which lets apt downgrade the handful of security-patched packages the base image
carries that the frozen archive does not have. `security.debian.org` is deliberately not used: it
currently returns 404 for the percent-encoded `+` that apt sends for versions like
`5.32.1-4+deb11u5`.

## Fuzzing

There are three libFuzzer targets in `fuzz/`:

| Target | What it does | What counts as a finding |
| --- | --- | --- |
| `fuzz_sff8472_roundtrip` | 128 bytes -> struct -> JSON -> struct -> 128 bytes | JSON violates its schema, any byte differs, or the tool cannot read back its own JSON |
| `fuzz_sff8636_roundtrip` | same for a 256 byte page 00h, comparing bytes 128-255 | as above |
| `fuzz_json_input` | arbitrary bytes through the `encode` path: FromJSON, assemble, pretty print, validate | crash, sanitizer report, or an exception not derived from `std::exception` |

The roundtrip targets assert the property this tool is built on: every programming, valid or not,
survives the JSON representation unchanged. Both fiber and copper interpretation are exercised per
input, because several bytes have two meanings. The intermediate JSON must also validate against
the handwritten schema; schema disagreements are fuzz findings. Schemas are loaded once per process
from this checkout, so restart a fuzzer after editing a schema.

For `fuzz_json_input` a rejected input is the expected outcome, so exceptions are caught. Only
memory errors and inputs that escape the CLI's error handling are bugs.

### JSON schema contract tests

The schemas remain handwritten public documentation. C++ converters retain responsibility for
binary interpretation, checksums, and byte preservation. Schema validation is a development-only
dependency and does not change the release CLI's input validation.

For a normal, unsanitized contract-test build:

```
conan install . --output-folder build --build=missing -s build_type=Release -o "&:schema_tests=True"
cmake -S . -B build -DCMAKE_TOOLCHAIN_FILE=build/conan_toolchain.cmake -DCMAKE_BUILD_TYPE=Release -DTRANSCEIVERTOOL_SCHEMA_TESTS=ON
cmake --build build --config Release --parallel
ctest --test-dir build -C Release --output-on-failure
```

CI runs the same checks on Linux, macOS, and Windows. The deterministic tests reuse the two
roundtrip fuzz targets with all 256 uniform byte patterns and 128 reproducible mixed patterns,
in both fiber and copper modes. Curated JSON input cases independently check schema and parser
acceptance of raw bytes, known names, checksum alternatives, vendor strings, missing fields,
and signaling-rate boundaries. These checks detect drift; they do not prove equivalence for all
possible JSON inputs. Base64 syntax is checked, while decoded lengths and other semantic rules
remain enforced by the converters.

The assigned-value tables used by C++ also supply the corresponding schema enum lists. To refresh
those lists after changing a table, run:

```
python3 tests/schema_enums.py build/tests/schema_enums --write
```

For a Windows multi-configuration build, the executable is `build/tests/Release/schema_enums.exe`.
Without `--write`, this checks for drift without editing anything (as CTest does). Only the enum
arrays are generated; surrounding definitions, descriptions, and formatting remain handwritten.
Regenerate the fuzz dictionary below after changing schema strings.

### Building

Fuzzing needs Clang, and the dependencies have to be built with the same instrumentation, so it uses
its own profile and its own output folder. This does not disturb a normal `build/` directory.

```
conan config install conan-profiles
conan install . --output-folder build-fuzzing --build=missing -pr:a default -pr:a TransceiverTool-fuzzing
cmake -DTRANSCEIVERTOOL_BUILD_FUZZERS=ON --preset conan-relwithdebinfo
cmake --build --preset conan-relwithdebinfo
```

`TRANSCEIVERTOOL_BUILD_FUZZERS` is OFF by default and refuses to configure with a non-Clang compiler.
Turning it on also puts ASan/UBSan on the `TransceiverTool` binary itself, which is useful for
reproducing a finding through the CLI but is not a build you want to ship.

Note that the first `conan install` rebuilds `fmt` from source with the sanitizers, which takes a
while. The profile lists those flags in `tools.info.package_id:confs`; without that conan would
silently reuse a cached, uninstrumented `fmt`.

### Running

```
mkdir -p fuzz/corpus/sff8472 fuzz/corpus/sff8636 fuzz/corpus/json
./build-fuzzing/fuzz/fuzz_sff8472_roundtrip fuzz/corpus/sff8472
./build-fuzzing/fuzz/fuzz_sff8636_roundtrip fuzz/corpus/sff8636
./build-fuzzing/fuzz/fuzz_json_input fuzz/corpus/json -dict=fuzz/json_input.dict
```

Seeding the corpora first helps a lot - any real transceiver dump works for the roundtrip targets,
and the JSON target should be seeded with output from `TransceiverTool decode --json-to-file`.
`-max_total_time=60` bounds a run, `-jobs=$(nproc)` parallelises one. Corpora and the `crash-*`
artifacts libFuzzer writes are gitignored.

`fuzz_json_input` gets a dictionary because the JSON keys are long literal strings that a mutator
will never produce on its own. Regenerate `fuzz/json_input.dict` from the schemas after changing the
JSON format:
...
def walk(n):
    if isinstance(n,dict):
        for k,v in n.items():
            if k=="properties" and isinstance(v,dict): keys.update(v.keys())
            if k=="const" and isinstance(v,str): keys.add(v)
            if k=="enum" and isinstance(v,list): keys.update(x for x in v if isinstance(x,str))
            walk(v)
    elif isinstance(n,list):
        for x in n: walk(x)
for f in ["schemas/SFF-8472_LowerA0h.json","schemas/SFF-8636_Upper00h.json"]: walk(json.load(open(f)))
keys.update(["SFF-8472 Rev 12.4 Lower Page A0h","SFF-8636 Rev 2.11 Upper Page 00h","Type","byteValue","auto","N/A"])
out=["# SPDX-FileCopyrightText: 2024-2026 Robin Christ",
     "# SPDX-License-Identifier: MPL-2.0", "",
     "# libFuzzer dictionary for fuzz_json_input.",
     "# Generated from schemas/*.json - the key names are far too long for the mutator to find on its own.",
     "# Regenerate with the snippet in the fuzzing section of README.MD.",""]
for k in sorted(k for k in keys if k):
    out.append('"%s"' % k.replace("\\","\\\\").replace('"','\\"'))
open("fuzz/json_input.dict","w").write("\n".join(out)+"\n")
EOF
```

### Reproducing a finding

libFuzzer writes the offending input next to the binary. Feed it back to the same target:

```
./build-fuzzing/fuzz/fuzz_json_input crash-<hash>
```

For the roundtrip targets the crash artifact is a plain binary programming, so it can also be fed
straight to the CLI, which prints the same byte-level diff:

```
./build-fuzzing/TransceiverTool roundtrip-test --binary-type 8636-256b --from-file crash-<hash>
```
