...
namespace TransceiverTool::Standards::SFF8636 {
    struct SFF8636_Upper00h {
        ...
        Fibre_Channel_Link_Length_And_Transmitter_Technology
            byte_135_fibre_channel_link_length_and_transmitter_technology;
        ...
        // Byte 142: Length (SMF)
        // Link length supported at the signaling rate in byte 140 or page 00h byte 222, for SMF fiber in km. A value of
        // 1 shall be used for reaches from 0 to 1 km.
        unsigned char byte_142_length_smf_in_kilometers;

        // Byte 143: Length (OM3 50 um)
        // Link length supported at the signaling rate in byte 140 or page 00h byte 222, for EBW 50/125 um fiber (OM3),
        // units of 2 m
        unsigned char byte_143_length_om3_in_2m;

        // Byte 144: Length (OM2 50 um)
        // Link length supported at the signaling rate in byte 140 or page 00h byte 222, for 50/125 um fiber (OM2),
        // units of 1 m
        unsigned char byte_144_length_om2_in_m;

        // Byte 145: Length (OM1 62.5 um) or Copper Cable Attenuation
        // Link length supported at the signaling rate in byte 140 or page 00h byte 222, for 62.5/125 um fiber (OM1),
        // units of 1 m, or copper cable attenuation in dB at 25.78 GHz.
        ...
        // Length of passive or active cable assembly (units of 1 m) or link length supported at the signaling rate in
        // byte 140 or page 00h byte 222, for OM4 50/125 um fiber (units of 2 m) as indicated by Byte 147. See 6.3.12
        ...
        // Nominal laser wavelength (wavelength=value/20 in nm) or copper cable attenuation in dB at 2.5 GHz (Byte 186)
