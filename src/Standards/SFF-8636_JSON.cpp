...
//############
    ...
    void SFF8636_Upper00hFromJSON(const nlohmann::json& j, SFF8636_Upper00h& programming) {
        ...
        if(!CC_BASEVal.has_value()) programming.byte_191_CC_BASE = calculateCC_BASEChecksum(binaryBuffer.data());
        if(!CC_EXTVal.has_value()) programming.byte_223_CC_EXT = calculateCC_EXTChecksum(binaryBuffer.data());
    }   
}