...
namespace TransceiverTool::Standards::SFF8636 {
    ...
    unsigned char calculateCC_BASEChecksum(unsigned char const* bytes) {
        ...
        // The check code is a 1-byte code that can be used to verify that the first 63 bytes of serial information in
        // the free side device are valid. The check code shall be the low order 8 bits of the sum of the contents of
        // all the bytes from 128 to 190, inclusive.
    ...
    unsigned char calculateCC_EXTChecksum(unsigned char const* bytes) {
        ...
        // The check code is a 1-byte code that can be used to verify that the first 32 bytes of extended serial
        // information in the free side device is valid. The check code shall be the low order 8 bits of the sum of the
        // contents of all the bytes from 192 to 222, inclusive.
