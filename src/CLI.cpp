...
namespace TransceiverTool {
    ...
    std::vector<std::byte> collectInput(BinaryType binaryType, const std::optional<std::string>& filePath) {
        ...
        switch(binaryType) {
            case BinaryType::SFF_8472_128b:
                numBytesToRead = 128;
                break;
            case BinaryType::SFF_8636_128b:
                numBytesToRead = 128;
                break;
            case BinaryType::SFF_8636_256b:
                numBytesToRead = 256;
                break;
            default:
        ...
        if(filePath.has_value()) {
            ...
            try {
                ...
                throw std::runtime_error(
                    fmt::format("Failed to open path {}, exception {}", filePath.value(), e.what())
                );
            }
            ...
            try {
                ...
                if(inStream.gcount() < numBytesToRead) {
                    throw std::runtime_error(
                        fmt::format(
                            "Failed to read enough bytes from path {}, exception {}", filePath.value(), e.what()
                        )
                    );
                }
            }
            ...
            try {
                ...
                while((readBytes =
                           std::fread(buffer.data() + totalReadBytes, 1, buffer.size() - totalReadBytes, stdin)) > 0) {
    ...
    std::vector<std::byte> collectInput(const std::optional<std::string>& filePath) {
        ...
        if(filePath.has_value()) {
            ...
            try {
                ...
                tempBuffer.resize(32 * 1024);  // 32kByte should be enough
                static_assert(sizeof(std::byte) == 1);
    ...
    class DecodeCommand {
    public:
        DecodeCommand(CLI::App& app) { initCommand(app); }

        CLI::App* subcommand;

    private:
        void initCommand(CLI::App& app) {
            CLI::App* decodeSubcommand = app.add_subcommand(
                "decode", "Decode / parse binary transceiver programming to JSON and/or pretty print"
            );
            ...
            decodeSubcommand
                ->add_option(
                    ...
                    "8636-128b: SFF-8636 (QSFP/+/28) and 128 byte, aka upper page 00h or the last 128 bytes of page "
                    "00h or\n"
                    "8636-256b: SFF-8636 (QSFP/+/28) and 256 byte, aka full page 00h and first 128 bytes ignored / "
                    "zeroed"
                )
                ->required()
                ->transform(CLI::CheckedTransformer(BinaryTypeMap));

            decodeSubcommand->add_flag(
                "--fiber-mode,!--copper-mode",
                fiberMode,
                "Specify meaning of bytes that can have multiple meanings, default fiber-mode"
            );
            decodeSubcommand->add_option(
                "--from-file", fromFile, "File path from which the binary shall be read instead of stdin"
            );
            decodeSubcommand->add_flag(
                "--pretty-print", prettyPrint, "Pretty-prints the decoded / parsed binary programming to stdout"
            );
            decodeSubcommand->add_flag(
                "--json-to-stdout", JSONToStdout, "Print the decoded / parsed binary programming as JSON to stdout"
            );
            decodeSubcommand->add_option(
                "--pretty-print-to-file",
                prettyPrintToFile,
                "Write the pretty-print of the decoded / parsed binary programming to a file"
            );
            decodeSubcommand->add_option(
                "--json-to-file", JSONToFile, "Write the JSON of the decoded / parsed binary programming"
            );
        ...
        void runCommand() {
            ...
            if(binaryType == BinaryType::SFF_8472_128b) {
                auto parsedStruct = TransceiverTool::Standards::SFF8472::parseBytesToStruct(
                    reinterpret_cast<unsigned char const*>(buffer.data())
                );

                if(prettyPrint || prettyPrintToFile.has_value()) {
                    prettyPrintStr =
                        TransceiverTool::Standards::SFF8472::prettyPrintProgramming(parsedStruct, fiberMode);
                }
                ...
                auto parsedStruct = TransceiverTool::Standards::SFF8636::parseBytesToStruct(
                    reinterpret_cast<unsigned char const*>(buffer.data())
                );

                if(prettyPrint || prettyPrintToFile.has_value()) {
                    prettyPrintStr =
                        TransceiverTool::Standards::SFF8636::prettyPrintProgramming(parsedStruct, fiberMode);
                }
            ...
            if(prettyPrintToFile.has_value()) {
                try {
                    ...
                    std::cerr << "Failed to write Pretty Print to path " << prettyPrintToFile.value() << " because "
                              << e.what() << "\n";
                }
            }
    ...
    class EncodeCommand {
    public:
        EncodeCommand(CLI::App& app) { initCommand(app); }

        CLI::App* subcommand;

    private:
        void initCommand(CLI::App& app) {
            CLI::App* encodeSubcommand =
                app.add_subcommand("encode", "Encode / assembly binary transceiver programming from JSON");
            ...
            encodeSubcommand->add_option(
                "--from-file", fromFile, "File path from which the JSON shall be read instead of stdin"
            );
            encodeSubcommand->add_option(
                "--to-file", toFile, "File path to which the binary shall be written instead of stdout"
            );
            encodeSubcommand->add_flag(
                "--validate", validate, "Try to validate programming, don't write file if any errors occur"
            );
        ...
        void runCommand() {
            ...
            if(binaryType == BinaryType::SFF_8472_128b) {
                if(type != Standards::SFF8472::SFF8472_JSON_TYPE) {
                    throw std::runtime_error(
                        fmt::format(
                            "JSON has wrong type, expected {} but is {}", Standards::SFF8472::SFF8472_JSON_TYPE, type
                        )
                    );
                }
                ...
                if(validate) {
                    auto validationResult =
                        TransceiverTool::Standards::SFF8472::Validation::validateSFF8472_LowerA0h(programming);
                ...
                if(type != Standards::SFF8636::SFF8636_JSON_TYPE) {
                    throw std::runtime_error(
                        fmt::format(
                            "JSON has wrong type, expected {} but is {}", Standards::SFF8636::SFF8636_JSON_TYPE, type
                        )
                    );
                }
                ...
                if(validate) {
                    auto validationResult =
                        TransceiverTool::Standards::SFF8636::Validation::validateSFF8636_Upper00h(programming);
            ...
            if(toFile.has_value()) {
                try {
                    ...
                    std::cerr << "Failed to write reassembled binary to path " << toFile.value() << " because "
                              << e.what() << "\n";
                }
            }
        }
    ...
    public:
        PrettyPrintJSONCommand(CLI::App& app) { initCommand(app); }

        CLI::App* subcommand;

    private:
        void initCommand(CLI::App& app) {
            CLI::App* prettyPrintJSONSubcommand =
                app.add_subcommand("pretty-print-json", "Pretty print a JSON file of a decoded programming");
            subcommand = prettyPrintJSONSubcommand;

            prettyPrintJSONSubcommand->add_flag(
                "--fiber-mode,!--copper-mode",
                fiberMode,
                "Specify meaning of bytes that can have multiple meanings, default fiber-mode"
            );
            prettyPrintJSONSubcommand->add_option(
                "--from-file", fromFile, "File path from which the JSON shall be read instead of stdin"
            );
            prettyPrintJSONSubcommand->add_option(
                "--to-file", toFile, "File path to which the pretty print shall be written instead of stdout"
            );
    ...
    public:
        ValidateJSONCommand(CLI::App& app) { initCommand(app); }

        CLI::App* subcommand;

    private:
        void initCommand(CLI::App& app) {
            CLI::App* validateJSONSubcommand =
                app.add_subcommand("validate-json", "Validate a JSON file of a decoded programming");
            subcommand = validateJSONSubcommand;

            validateJSONSubcommand->add_option(
                "--from-file", fromFile, "File path from which the JSON shall be read instead of stdin"
            );
    ...
    public:
        RoundtripTestCommand(CLI::App& app) { initCommand(app); }

        CLI::App* subcommand;

    private:
        void initCommand(CLI::App& app) {
            CLI::App* roundtripSubcommand =
                app.add_subcommand("roundtrip-test", "Perform a roundtrip test (Binary -> JSON -> Binary)");
            ...
            roundtripSubcommand->add_option(
                "--from-file", fromFile, "File path from which the binary shall be read instead of stdin"
            );
            ...
            try {
                if(binaryType == BinaryType::SFF_8472_128b) {
                    ...
                    auto test = [&buffer = std::as_const(buffer)](bool fiberMode) {
                        ...
                        try {
                            TransceiverTool::Standards::SFF8472::SFF8472_LowerA0hFromJSON(
                                nlohmann::json::parse(serialised), programmingRoundtrip
                            );
                        } catch(const std::exception& e) {
                            throw std::runtime_error(
                                fmt::format(
                                    "SFF8472_LowerA0hFromJSON for fiberMode = {} failed with exception {}",
                                    fiberMode,
                                    e.what()
                                )
                            );
                        ...
                        for(int index = 0; index < 128; ++index) {
                            if(std::byte{reassembled[index]} != buffer[index]) {
                                ...
                                std::cerr << fmt::format(
                                    "fiberMode = {}, bytes at index {} different. Should be {:#04x} but is {:#04x}\n",
                                    fiberMode,
                                    index,
                                    buffer[index],
                                    reassembled[index]
                        ...
                        if(reassembleError) {
                            throw std::runtime_error("comparison between original bytes and reassembled bytes failed");
                        }
                    };
                    ...
                    auto test = [&buffer = std::as_const(buffer)](bool fiberMode) {
                        ...
                        try {
                            TransceiverTool::Standards::SFF8636::SFF8636_Upper00hFromJSON(
                                nlohmann::json::parse(serialised), programmingRoundtrip
                            );
                        } catch(const std::exception& e) {
                            throw std::runtime_error(
                                fmt::format(
                                    "SFF8636_LowerA0hFromJSON for fiberMode = {} failed with exception {}",
                                    fiberMode,
                                    e.what()
                                )
                            );
...
int main(int argc, char** argv) {
    ...
    CLI::App app{fmt::format(
        "TransceiverTool v{} - Encode / Assemble from JSON, Decode / Parse to JSON and Pretty Print SFF-8472 SFP/+/28 "
        "and SFF-8636 QSFP/+/28 programmings / EEPROMs!",
    ...
    if(!decodeCommand.subcommand->parsed() && !encodeCommand.subcommand->parsed() &&
       !prettyPrintJSONCommand.subcommand->parsed() && !validateJSONCommand.subcommand->parsed() &&
       !roundtripCommand.subcommand->parsed()) {
